...
function Organizations() {
  ...
  useEffect(() => {
    const fetchOrganizations = async () => {
      try {
        if (search === '') {
          const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/org/show_accredited_orgs`);
          setOrganizations(response.data);
        } else {
          const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/org/show_accredited_orgs/${search}`);
          setOrganizations(response.data);
        }
      } catch (err) {
        console.log(err);
