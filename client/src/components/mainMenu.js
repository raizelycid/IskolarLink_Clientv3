...
const MainMenu = ({imgSrc, username}) => {
    ...
    useEffect(() => {
        axios.get(`${process.env.BACKEND_URL}/student/accreditation_status`).then((response) => {
    ...
    const changeMainMenu = () => {
        setActiveMenu('main');
        axios.post(`${process.env.BACKEND_URL}/menu/`, {menu: 'main'})
      }
    
      const changeCosoaMenu = () => {
        setActiveMenu('cosoa');
        axios.post(`${process.env.BACKEND_URL}/menu/`, {menu: 'cosoa'})
      }
    
      const changeWebAdminMenu = () => {
        setActiveMenu('webadmin');
        axios.post(`${process.env.BACKEND_URL}/menu/`, {menu: 'webadmin'})
      }
    ...
    return(
    <NavDropdown title={<>{imgSrc ? <img src={`${process.env.BACKEND_URL}/images/${imgSrc}`} alt="Profile Picture" width="40" height="40" className="rounded-circle" /> : <FontAwesomeIcon icon={faUser}/>} <span className='text-dark'>Hi, {username}!</span></>} id="basic-nav-dropdown" className="text-dark" renderMenuOnMount={true}>
        ...
        <NavDropdown.Item onClick={() => {
            axios.post(`${process.env.BACKEND_URL}/auth/logout`)
            .then((response) => {
                if(response.data.error){
                    alert(response.data.error);
                }
